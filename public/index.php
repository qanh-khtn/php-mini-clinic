...
$dotenv->safeLoad();
$dotenv->required(['APP_NAME', 'APP_ENV', 'APP_DEBUG', 'APP_URL', 'CLINIC_NAME', 'MAX_REGISTRATIONS_PER_REQUEST']);
...
$appointments = require dirname(__DIR__) . '/src/Data/appointments.php';
...
if ($path === '/appointments' && $method === 'GET') {
    (new AppointmentController())->index($appointments);
}

if ($path === '/appointments' && $method === 'HEAD') {
    (new AppointmentController())->head();
}

if ($path === '/appointments' && !in_array($method, ['GET', 'HEAD'], true)) {
    Response::json(405, ['error' => 'Method Not Allowed'], ['Allow' => 'GET, HEAD']);
}

if ($path === '/registrations' && $method === 'POST') {
    (new RegistrationController())->store($appointments, $config);
}
